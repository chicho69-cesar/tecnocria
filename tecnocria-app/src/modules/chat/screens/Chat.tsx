...
export default function Chat() {
  const { category } = useParams<{ title: string; category: string }>()
  const { room, joinRoom, leaveRoom, sendMessage } = useSocketManager()
  const { user } = useAuth()
  const { conversation, isLoading } = useMessages(category)
  ...
  const handleSendMessage = useCallback(
    (messages: IMessage[] = []) => {
      if (room) {
        sendMessage(messages[0].text, 'Message')
      }
